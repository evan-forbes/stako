...
- **Important** — `src/daemon.zig:298-307` — **Token file generated even when the host is non-loopback.** `local_token.ensureAndLoad` runs on line 303; the loopback check is on line 307. A `start({ host = "0.0.0.0" })` that fails fast with `error.NotLoopbackHost` still wrote `.stako/local_token` if it was absent. Plan step 5 says the daemon should "refuse to bind on non-loopback addresses — explicit check, with a test" and acceptance criterion 5 ties to "fails fast with a clear error". A side effect on a fail-fast path is at minimum surprising. Move the loopback check to the top of `start()` (or at least before any disk write).
...
- **Token file perms on disk** — no test asserts the generated `.stako/local_token` actually has mode `0o600`. The init-flow tests check it; the daemon-generation path does not.
