...
//!
//! Path: `<notes-root>/state/runtime/<stack>/<id>.toml`.
//!
...
pub fn dirPath(allocator: std.mem.Allocator, notes_root_abs: []const u8) ![]u8 {
    return std.fs.path.join(allocator, &.{ notes_root_abs, "state", "runtime" });
}

pub fn stackDirPath(allocator: std.mem.Allocator, notes_root_abs: []const u8, stack: []const u8) ![]u8 {
    return std.fs.path.join(allocator, &.{ notes_root_abs, "state", "runtime", stack });
}
...
pub fn filePath(allocator: std.mem.Allocator, notes_root_abs: []const u8, stack: []const u8, id: []const u8) ![]u8 {
    ...
    return std.fs.path.join(allocator, &.{ notes_root_abs, "state", "runtime", stack, file_name });
}
...
/// Walk `state/runtime/` and return one Orphan per file found. Caller
