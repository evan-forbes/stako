...
- `src/local_token.zig:51` — `createFile(.mode = 0o600)` on Linux honors the mode bit, but if the parent dir was created by `makePath` it inherits the user's umask. The design doc requires the directory `.organo/credentials/` to be `0700`; `.organo/` itself isn't specified, but the local_token sits there. The token file mode is right; just confirm nothing else relies on `.organo/` being 0700.
...
- `organo daemon stop` mid-execution semantics (SIGINT → grace → SIGTERM to children) — design says this lands with the harness layer in milestone 6; milestone 3 daemon has no children to clean up.
...
- **`.organo/local_token` exists with perms 0600, is stable across restarts, validatable for future mutation requests** — MET. `init.zig` writes the token with `posix_mode = 0o600`; `local_token.zig` ensures the file isn't rewritten on re-load; `Token.verify` does constant-time comparison. `daemon: token is generated/loaded on start and verifies` covers the daemon-side load.
...
5. **`organo daemon start` (bind, write PID, redirect logs, refuse running)** — MET (`src/cli.zig` `runDaemon` + `src/daemon.zig` `start`/`writePidFile`/`openLogFile`).
6. **`organo daemon stop` (SIGTERM, wait, remove PID)** — MET (`daemon.zig` `stop`).
7. **`organo daemon status`** — MET (`cli.zig` reports running/stopped + pid/port/uptime).
